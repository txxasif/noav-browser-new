...
│   ├── mem_guard.py        -> Dynamic RAM watchdog & concurrency clamp  │
...
| **Low RAM Guard Alert** | Available system RAM is under 1500 MB. | Close heavy background programs. Memory Guard automatically clamps concurrency to protect system stability. |
...
| **Parallel count ignored — only ~7 windows start** | Hard cap was 10 (UI `max`, server, worker); the RAM guard also clamped to `(freeMB-1000)/600` | Cap is now 50 and the guard **warns only** (never silently reduces the requested count). 20 headless Chromes need ~12-14 GB free RAM; the engine's watchdog still sheds/stops under real pressure |
...
| **Dashboard mail/captcha choice had no effect** | `server.js` read `opts.mail` / `opts.captcha` while the UI sends `mail_provider` / `captcha_mode` | Server now accepts both; `rotate` added (round-robins mail.td → TempMail.fish → Guerrilla → mail.tm per account for domain diversity) |
...
| **Engine refuses to start on 4-8 GB machines ("only N MB RAM available")** | `mem_guard` used a fixed 4 GB reserve, so the start floor was ~4.7 GB free — a low-end laptop could never start even one browser | Reserve now scales to ~35% of physical RAM (clamped 1000 MB–4000 MB); a 4 GB machine with ~2.5 GB free can run one browser. Dashboard default Parallel lowered to 1 |
...
- [x] Dashboard: Meta/Instagram sidebar sheets, per-account Cookie column + JSON export, contextual Combo export (Meta = `email|password`; IG = `username|password|cookies`), Global Password at the top of the sidebar (click → modal → Save, persisted in `data/settings.json`), and Parallel up to 50 (RAM guard warns, never clamps).
- [x] Anti-detect parity with meta_auto_ai: per-profile device identity (`device_identity` seeded from the profile dir) shared by the UA and the injected fingerprint — verified in-browser (model / hardwareConcurrency / deviceMemory / maxTouchPoints agree and differ per profile); captcha waits env-tunable (30s cap / 15s bail); `rotate` mail provider; dashboard mail/captcha picks now reach the worker.
- [x] 2026-09-23 release rebuilt: Portable 501.56 MB (SHA-256 `a5c38fe20f9e1901c9dfb6429ddc0b58558b4ea21ced5eb11a95ebcdf0c187ef`) + Patch 54.98 MB / 116 files (SHA-256 `3d21fd01f9852c7ff17a772ac80fb5a9d1368bf99651aee667565c1edcfade92`).
...
- [x] Installer parity restored: `setup.iss` ships `runner.py`, `selfies/`, `img/`, `Update.bat`; build now validates every installer source and syncs `windows_dist/installer/`.
